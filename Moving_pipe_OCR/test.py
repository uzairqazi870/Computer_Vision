...
class pipe_ocr:
    ...
    def display_output(self, pipe_details):
        x = self.x + self.init_x
        y = self.y + self.init_y
        w = self.w 
        h = self.h
        
#       cv2.putText(self.frame_orig, pipe_details, (x, y - 20), cv2.FONT_HERSHEY_COMPLEX, 
#                         1, (255,0,255), 2, cv2.LINE_4)
...
if __name__ == '__main__':
    ...
    while cap.isOpened():
        ...
        if roi is not None:
            ...
            try:
                x, y, w, h = pipe.display_output(pipe_details)
                cv2.rectangle(frame, (x, y), (x + w, y + h), (255, 0, 255), 2)
            except:
                pass
        
        if pipe_details is not None:
            cv2.putText(frame, pipe_details, (x, y-100), cv2.FONT_HERSHEY_COMPLEX, 1, (255,0,255), 2, cv2.LINE_4)
