...
class pipe_ocr:
    def __init__(self, frame):
        ...
        self.frame = frame[self.init_y : self.init_h, self.init_x : (4*self.init_w)//5]   

    def process(self):
        
        self.gray = cv2.cvtColor(self.frame, cv2.COLOR_BGR2GRAY)
        self.gray = cv2.bilateralFilter(self.gray, 3, 4, 2)
    ...
    def verify(self, contours):
        for c in contours:
            if 3500 < cv2.contourArea(c) < 4000: 
                ...
                if (250 < self.w < 260) and (30 < self.h < 40) :
                    
                    cropped = self.gray[self.y:self.y + self.h, self.x:self.x + self.w]
                    cropped = cv2.rotate(cropped, cv2.ROTATE_180)

                    cv2.rectangle(self.frame, (self.x, self.y), (self.x + self.w, self.y + self.h), (255, 0, 255), 2)
